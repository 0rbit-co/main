...
- `Send` is the function that takes several tags as the arguments and creates a message on the ao: 
    ...
    | ["X-Headers"] | The _forwarded-tag_ which contains the headers to be sent in the POST request. |
...
```bash
aos postRequest --load Post-Request.lua
```
...
### Call the Handler 
    ...
    ```bash
    Send({ Target= ao.id, Action="Post-Request" })
    ```
    <details>
    <summary>
