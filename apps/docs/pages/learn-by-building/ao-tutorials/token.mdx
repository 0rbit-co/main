...
- Within the `Handlers.utils` module, there's a helpful function called **`hasMatchingTag(name, value)`** which returns a function that requires a message argument. These function compares the Tag Name and Value, if they are equal then it invokes the handle function.
  ...
  - When you send a message to the token process, like **`Send({ Target = ao.id, Action = “Info”})`**, it's like dropping a request into a mailbox.
...
Before we initiate the transfer, we make sure to check if the sender has enough balance to cover it and if it is true then we update the sender’s and recipient's balances accordingly.
